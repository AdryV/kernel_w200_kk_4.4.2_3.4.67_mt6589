...
//mly add 
s32 gtp_read_cfg_version(struct i2c_client *client, u16 *cfgversion)
{
    s32 ret = -1;
    s32 i;
    u8 buf[3] = {0};
    ...
    GTP_INFO("CFG VERSION: %02x",buf[2]);  

    return ret;
}

void get_tp_fw_id(unsigned char *data,unsigned char *cfgdata)
{
    s32 ret = -1;
        
    ret=gtp_read_version(i2c_client_point,data);
    if (ret < 0)
    {
        GTP_ERROR("Read version failed.");
    } 
#if 1
    ret=gtp_read_cfg_version(i2c_client_point,cfgdata);
    if (ret < 0)
    {
