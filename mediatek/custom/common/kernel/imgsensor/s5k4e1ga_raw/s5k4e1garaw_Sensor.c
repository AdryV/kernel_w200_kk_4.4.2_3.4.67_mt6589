...
static u8 S5K4E1GA_WRITE_ID = 0; 
static u8 S5K4E1GA_READ_ID = 0; 
...
static void S5K4E1GAPreviewSetting(void)
{
 ...
 //
 S5K4E1GA_write_cmos_sensor(0x3110,0x10);//modify bit4 0--> 1
 S5K4E1GA_write_cmos_sensor(0x3117,0x06);
S5K4E1GA_write_cmos_sensor(0x3119,0x0A);
S5K4E1GA_write_cmos_sensor(0x311A,0xAA);
...
SENSORDB("[S5K4E1]CaptureSetting enter\n ");
 ...
 S5K4E1GA_write_cmos_sensor(0x30E2,0x01);
 //
 S5K4E1GA_write_cmos_sensor(0x3110,0x10);//modify 0x00 --> 0x10
 S5K4E1GA_write_cmos_sensor(0x3117,0x06);
S5K4E1GA_write_cmos_sensor(0x3119,0x0A);
S5K4E1GA_write_cmos_sensor(0x311A,0xAA);
 /*// Integration setting
...
UINT32 S5K4E1GAGetSensorID(UINT32 *sensorID) 
{
	...
	#endif
	S5K4E1GA_WRITE_ID = 0x6C; 
        S5K4E1GA_READ_ID = 0x6D;
    ...
    do {
        *sensorID = (S5K4E1GA_read_cmos_sensor(0x0000)<<8)|S5K4E1GA_read_cmos_sensor(0x0001);        
        ...
        {      	
            printk("[S5K4E1GA] WRITE_ID = 0x6C [S5K4E1GAGetSensorID] Sensor ID = 0x%04x\n", *sensorID);
            break; 
        }
        printk("[S5K4E1GA] WRITE_ID = 0x6C [S5K4E1GAGetSensorID] Read Sensor ID Fail = 0x%04x\n", *sensorID);
        retry--; 
    } while (retry > 0);
    if (*sensorID != S5K4E1GA_SENSOR_ID) 
    {
        retry = 3; 
        S5K4E1GA_WRITE_ID = 0x20; 
        S5K4E1GA_READ_ID = 0x21;
        do {
                *sensorID = (S5K4E1GA_read_cmos_sensor(0x0000)<<8)|S5K4E1GA_read_cmos_sensor(0x0001);        
                if (*sensorID == S5K4E1GA_SENSOR_ID)
                {      	
                    printk("[S5K4E1GA] WRITE_ID = 0x20 [S5K4E1GAGetSensorID] Sensor ID = 0x%04x\n", *sensorID);
                    break; 
                }
                printk("[S5K4E1GA] WRITE_ID = 0x20 [S5K4E1GAGetSensorID] Read Sensor ID Fail = 0x%04x\n", *sensorID);
                retry--; 
                } while (retry > 0);
                if (*sensorID != S5K4E1GA_SENSOR_ID) 
                { 
...
#ifdef BIRD_S5K4E1GA_HV_MIRROR
	S5K4E1GASetFlipMirror(IMAGE_HV_MIRROR);
#elif defined(BIRD_S5K4E1GA_H_MIRROR)
	S5K4E1GASetFlipMirror(IMAGE_H_MIRROR);
#elif defined(BIRD_S5K4E1GA_V_MIRROR)
	S5K4E1GASetFlipMirror(IMAGE_V_MIRROR);
#else
	S5K4E1GASetFlipMirror(IMAGE_NORMAL);
#endif
	//set dummy
	spin_lock(&s5k4e1garaw_drv_lock);
	s5k4e1ga.DummyPixels=0;
...
#ifdef BIRD_S5K4E1GA_HV_MIRROR
	s5k4e1ga.imgMirror = IMAGE_HV_MIRROR;
#elif defined(BIRD_S5K4E1GA_H_MIRROR)
	s5k4e1ga.imgMirror = IMAGE_H_MIRROR;
#elif defined(BIRD_S5K4E1GA_V_MIRROR)
	s5k4e1ga.imgMirror = IMAGE_V_MIRROR;
#else
	s5k4e1ga.imgMirror = IMAGE_NORMAL;
#endif
   ...
   // pSensorInfo->SensorISOBinningInfo.ISOBinningInfo[ISO_1600_MODE].BinningEnable=TRUE;
    ...
    switch (ScenarioId)
    {
        ...
        //case MSDK_SCENARIO_ID_CAMERA_CAPTURE_MEM:
            ...
            pSensorInfo->SensorPixelClockCount= 3;
            pSensorInfo->SensorDataLatchCount= 2;
            pSensorInfo->SensorGrabStartX = S5K4E1GA_FULL_X_START;//4*S5K4E1GA_FULL_X_START;	//2*S5K4E1GA_IMAGE_SENSOR_PV_STARTX; 
            pSensorInfo->SensorGrabStartY = S5K4E1GA_FULL_Y_START;//2*S5K4E1GA_FULL_Y_START;	//2*S5K4E1GA_IMAGE_SENSOR_PV_STARTY;          			
            pSensorInfo->SensorWidthSampling = 0;  // 0 is default 1x
            pSensorInfo->SensorHightSampling = 0;   // 0 is default 1x
            pSensorInfo->SensorPacketECCOrder = 1;
            break;
