...
//extern void synaptics_gpio_init(void);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	return tpd_i2c_read_data(rmi4_data, addr, data, length);
#if 0
	int retval;
	unsigned char retry;
	unsigned char buf;
	...
	dev_dbg(&rmi4_data->i2c_client->dev,
			"%s(addr=0x%0x, len=%d): i2c={addr=0x%02x, name=%s, irq=0x%0x}\n",
			__func__, addr, length, rmi4_data->i2c_client->addr, rmi4_data->i2c_client->name, rmi4_data->i2c_client->irq);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	return tpd_i2c_write_data(rmi4_data, addr, data, length);
#if 0	
	int retval;
	unsigned char retry;
...
static void tpd_eint_handler(void)
{
	...
	queue_work(mtk_tpd_wq, &rmi4_data->work);
}
 ...
 */
static bool tpd_eint_initialized = false;
...
static void tpd_switch_power(bool enable)
{
...
*/
	...
	INIT_WORK(&rmi4_data->work, tpd_work_func);
	mtk_tpd_wq = create_singlethread_workqueue("mtk_tpd_wq");
	if (!mtk_tpd_wq)
	{
		dev_err(&client->dev,
			"%s: Error Could not create work queue mtk_tpd_wq: no memory",
			__func__);
		retval = -ENOMEM;
...
//	rmi4_data->irq = gpio_to_irq(platform_data->irq_gpio);
	rmi4_data->irq = CUST_EINT_TOUCH_PANEL_NUM;

//	dev_dbg(&client->dev,
//				"%s: gpio_to_irq(%d) => %d\n",
//				__func__, platform_data->irq_gpio, rmi4_data->irq);
...
static int synaptics_gpio_setup(int gpio, bool configure, int dir, int state)
{
        ...
        if (configure) {
        		...
        		pr_notice("%s(%i,%i,%i)-configure: skipping",
                                        __func__, gpio, dir, state);
#if 0
        		...
        		pr_notice("%s(%i)-free: skipping", __func__, gpio);
                //gpio_free(gpio);
        }
...
//static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(DRIVER_NAME, (0x38))};
...
static struct i2c_board_info __initdata bus4_i2c_devices[] = {
	{
...
static int tpd_local_init(void)
{
    TPD_DMESG("Synaptics DSX I2C Touch Driver (Built %s @ %s)\n", __DATE__, __TIME__);

    if(synaptics_rmi4_init()!=0)
    {
...
static int __init tpd_driver_init(void)
{
        ...
        i2c_register_board_info(0, &bus4_i2c_devices, 1);
        if(tpd_driver_add(&tpd_device_driver) < 0)
                TPD_DMESG("Error: Adding Synaptics driver failed\n");
        return 0;
}
...
MODULE_AUTHOR("Synaptics, Mediatek.");
MODULE_DESCRIPTION("Mediatek Synaptics DSX I2C Touch Driver");
