...
extern struct tpd_device *tpd;//add by wuhao virtual keys
// for DMA accessing
static u32 *gpwDMABuf_va = NULL;
static u32 gpwDMABuf_pa = NULL;
static u32 *gprDMABuf_va = NULL;
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	mutex_lock(&(rmi4_data->rmi4_io_ctrl_mutex));
        ...
        gprDMABuf_va = dma_alloc_coherent(&rmi4_data->input_dev->dev, 1024, &gprDMABuf_pa, GFP_KERNEL);
        if(!gprDMABuf_va){
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	mutex_lock(&(rmi4_data->rmi4_io_ctrl_mutex));
	rmi4_data->input_dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
	gpwDMABuf_va = dma_alloc_coherent(&rmi4_data->input_dev->dev, 1024, &gpwDMABuf_pa, GFP_KERNEL);
        if(!gpwDMABuf_va){
        	printk("[Error] Allocate DMA I2C Buffer failed!\n");
        }
	buf_va = gpwDMABuf_va;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	int x,y;//add by wuhao virtual keys 
	if (do_once) {
...
//add by wuhao virtual keys start
			#if 0
			touch_count++;
			input_report_key(rmi4_data->input_dev,
					f1a->button_map[button],
			...
			if (status==1)
			{
				switch (button)
				{
					...
					break;
					default:
						dev_dbg(&rmi4_data->i2c_client->dev,"%s, unknown button:%d\n",
				__func__,  button);
					break;
				}
				// generate touch event sequence
				input_report_key(tpd->dev,BTN_TOUCH, 1);
				input_report_abs(tpd->dev,ABS_MT_POSITION_X, x);
				input_report_abs(tpd->dev,ABS_MT_POSITION_Y, y);
				input_report_abs(tpd->dev,ABS_MT_TOUCH_MAJOR,5);
				input_report_abs(tpd->dev,ABS_MT_TRACKING_ID, button);
...
#ifdef TPD_HAVE_BUTTON
	//rmi4_data->sensor_max_y = rmi4_data->sensor_max_y * TPD_DISPLAY_HEIGH_RATIO / TPD_TOUCH_HEIGH_RATIO;//delete by wuhao virtual keys
#endif
	dev_dbg(&rmi4_data->i2c_client->dev,
...
/*	g_dev =&rmi4_data->input_dev->dev;
	unsigned char rgk_firmware[4];
	int rgk_num=3;
	
	struct synaptics_rmi4_data *rgk_data = dev_get_drvdata(g_dev);
	
	retval = synaptics_rmi4_i2c_read(rgk_data, 0x0032, rgk_firmware, sizeof(rgk_firmware));
	...
	len=sprintf(tp_info,"TP IC :%s,TP module :%s,TP I2C adr:0x%x,SW FirmWare:%02x%02x%02x%02x,Sample FirmWare:20140912","synaptics","Trust",client->addr,rgk_firmware[0], rgk_firmware[1], rgk_firmware[2], rgk_firmware[3]);
