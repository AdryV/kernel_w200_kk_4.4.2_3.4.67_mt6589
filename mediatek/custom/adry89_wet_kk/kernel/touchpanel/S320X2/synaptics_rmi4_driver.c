...
static int lcd_x = LCD_X;
static int lcd_y = LCD_Y;
...
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(DRIVER_NAME, (0x20))};		//add by lishengli 20130514
	...
	if (status & ts->fn11_mask) {	
		...
		//		finger_status = finger_status_reg[i / 4];
			...
			if (0x01==finger_status || 0x02 == finger_status) {
				...
				#endif
				ppt->x = ppt->raw_x * lcd_x / ts->f11_max_x;
				ppt->y = ppt->raw_y * lcd_y / ts->f11_max_y;
				
				tpd_down(ppt->x, ppt->y, ppt->z);
				CTP_DBG("TOUCH: finger index=:%d, rx=%d, ry=%d, x=%d, x=%d, z=%d\n",(i+1),ppt->raw_x, ppt->raw_y, ppt->x, ppt->y, ppt->z);
